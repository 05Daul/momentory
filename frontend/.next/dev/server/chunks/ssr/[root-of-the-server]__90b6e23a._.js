...
// TiptapEditor 컴포넌트
// ----------------------------------------------------------------------
const TiptapEditor = ({ content, onChange, onThumbnailChange })=>{
    const editor = (0, __TURBOPACK__imported__module__$5b$externals$5d2f40$tiptap$2f$react__$5b$external$5d$__$2840$tiptap$2f$react$2c$__esm_import$29$__["useEditor"])({
        ...
        onUpdate: ({ editor })=>{
            ...
            // ✅ [핵심 수정] 썸네일 추출 로직 (업데이트 시마다 첫 번째 이미지 URL을 전달)
            if (onThumbnailChange) {
                const content = editor.getJSON().content;
                // content가 배열인지 확인하여 undefined 오류 방지
                if (Array.isArray(content)) {
                    // 이미지 노드만 필터링
                    const imageNodes = content.filter((node)=>node.type === 'image');
                    if (imageNodes.length > 0) {
                        const firstImage = imageNodes[0]; // 타입 추론 보조
                        // ✅ [추가 수정] attrs와 src 속성이 존재하는지 안전하게 확인
                        if (firstImage && firstImage.attrs && firstImage.attrs.src) {
                            onThumbnailChange(firstImage.attrs.src);
                        } else {
                    ...
                    // 내용이 비어있어 content가 undefined인 경우
                    onThumbnailChange(null);
                }
            }
        },
    ...
    // 초기 로드 시 썸네일 URL 설정 (수정 모드에서 기존 데이터 로드 시 필요)
    (0, __TURBOPACK__imported__module__$5b$externals$5d2f$react__$5b$external$5d$__$28$react$2c$__cjs$29$__["useEffect"])(()=>{
        // ✅ [핵심 수정] 널 체크 및 안전한 썸네일 추출 로직 적용
        if (editor && onThumbnailChange) {
            const content = editor.getJSON().content;
            if (Array.isArray(content)) {
                const imageNodes = content.filter((node)=>node.type === 'image');
                if (imageNodes.length > 0) {
...
"[project]/src/component/blogService/WritePage.tsx [ssr] (ecmascript)", (() => {{

throw new Error("An error occurred while generating the chunk item [project]/src/component/blogService/WritePage.tsx [ssr] (ecmascript)\n\nCaused by:\n- CJS module can't be async.\n\nDebug info:\n- An error occurred while generating the chunk item [project]/src/component/blogService/WritePage.tsx [ssr] (ecmascript)\n- Execution of <ModuleChunkItem as EcmascriptChunkItem>::content_with_async_module_info failed\n- Execution of EcmascriptChunkItemContent::new failed\n- CJS module can't be async.");
...
//# sourceMappingURL=%5Broot-of-the-server%5D__90b6e23a._.js.map